...
export default function MembersPanel() {
  ...
  const [grantReminders, setGrantReminders] = useState([]);
  ...
  async function mutateMember(action, email) {
    ...
    try {
      ...
      if (action === "delete") {
        ...
        if (action === "grant") {
          setGrantReminders(current => (
            current.includes(data.member.email) ? current : [...current, data.member.email]
          ));
        }
      }
          ...
          <table style={{ width: "100%", minWidth: 920, borderCollapse: "collapse" }}>
            ...
            <tbody>
              {members.map(member => {
                ...
                const showReminder = grantReminders.includes(member.email);
                    ...
                    {showReminder && (
                      <tr>
                        <td colSpan={6} style={{ padding: "0 12px 10px", borderTop: "1px solid var(--border-hairline)" }}>
                          <div role="status" style={{ display: "flex", alignItems: "center", gap: 8, color: "var(--accent)", fontSize: 10, lineHeight: 1.45 }}>
                            <span>Also add this email to the Capex IQ Members list in Zero Trust so they can sign in.</span>
                            <button
                              type="button"
                              aria-label={`Dismiss reminder for ${member.email}`}
                              onClick={() => setGrantReminders(current => current.filter(email => email !== member.email))}
